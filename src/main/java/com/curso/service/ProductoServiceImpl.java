...
public class ProductoServiceImpl implements ProductoService {
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = GestionProductoException.class)
		...
		for (Producto p : productos) {
			if (p.getPrecioUnitario().doubleValue() > nuevoPrecio) {
				throw new GestionProductoException(p.getIdProducto(),
						"No se pudo cambiar el precio ya que el nuevo " + "precio no puede ser menor");
			}
